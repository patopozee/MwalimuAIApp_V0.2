...
# Assuming generate_lesson is exported from your app core module along with others
...
if st.sidebar.button("💬 Ask Mwalimu (Main Chat)", use_container_width=True):
    st.session_state.current_page = "Main Chat"
    st.rerun()
if st.sidebar.button("📝 Test & Learning Generators", use_container_width=True):
...
if st.session_state.current_page == "Main Chat":
    ...
    # --- AI STUDY PLAN SECTION IN CONTEXT VIEW ---
    st.markdown("---")
    st.subheader("🗓️ AI Personalized Study Plan")
...
# ==========================================
# PAGE VIEW MODE 2: GENERATORS WORKSPACE HUB 
# ==========================================
elif st.session_state.current_page == "Generators Hub":

    st.markdown("---")
    if st.button("⬅️ Return to Main Chat Dashboard", use_container_width=True):
        st.session_state.current_page = "Main Chat"
        st.rerun()
        
    st.markdown("---")
    
    # 3-tab layout incorporating the new AI Lessons Generator Tab
    ...
    with tab2:
        ...
        if st.button("Generate Flashcards", use_container_width=True):
            if flashcard_topic.strip() == "":
                ...
                # --- FIX: Stop execution if profile name is missing ---
                st.warning("Please create Student Profile in the sidebar first!")
            else:
    ...
    with tab3:
        ...
        if "lesson_content" in st.session_state and st.session_state.lesson_content:
            ...
            # --- CLEAN RENDERING FORCE ---
            lesson_text = st.session_state.lesson_content
            
            # Safe check: if the model wrapped it in code fences accidentally, strip them away
